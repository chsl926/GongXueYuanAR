...
namespace SimpleFramework.Manager {
    public class GameManager : LuaBehaviour {
        ...
        IEnumerator OnExtractResource() {
            ...
            if (Directory.Exists(dataPath + "lua/")) Directory.Delete(dataPath + "lua/", true);
        ...
        IEnumerator OnUpdateResource() {
            ...
            string dataPath = Util.DataPath;  //数据目录
            string url = Util.AppContentPath();
            string message = string.Empty;
            string newtUrl = url + "files.txt";
            if (Application.platform == RuntimePlatform.WindowsEditor || Application.platform == RuntimePlatform.WindowsPlayer)
                newtUrl = "file:// " + newtUrl;
            Debug.Log("LoadUpdate---->>>" + newtUrl);
            GUIDebug.info = "LoadUpdate---->>>" + newtUrl;
            WWW www = new WWW(newtUrl);
            yield return www;
            if (www.error != null) {
                Debug.Log(www.error );
            ...
            for (int i = 0; i < files.Length; i++) {
                ...
                if (canUpdate) {   //本地缺少文件
                    ...
                    WWW www1 = new WWW(fileUrl);
                    yield return www1;
                    if (www1.isDone)
                    {
                        File.WriteAllBytes(localfile, www1.bytes);
                    }
