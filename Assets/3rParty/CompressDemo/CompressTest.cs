...
public class CompressTest : MonoBehaviour {
    ...
    void OnGUI()
    {
        ...
        {
            UPKFolder.PackFolderAsync(Application.dataPath + "/Tank", Application.dataPath + "/Tank.upk", ShowProgress);
        }
        if (GUI.Button(new Rect(Screen.width / 2 + 70, 440, 140, 80), "UPK解包文件夹"))
        {
            UPKFolder.UnPackFolderAsync(Application.dataPath + "/" + "3rParty/CompressDemo" + "/Tank.upk", Application.dataPath + "/" + "3rParty/CompressDemo/", ShowProgress);
        }
